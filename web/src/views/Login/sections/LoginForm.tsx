...
function LoginForm(): JSX.Element {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        setIsLoading(true);
        try {
            // loginAction runs on the server: verifies credentials, signs the JWT,
            // sets an httpOnly cookie (for SSR middleware), and returns user+token
            // so the client can also persist to localStorage for the REST auth flow.
            const { user, token } = await loginAction(email, password);
            // PublicUser is a superset of AuthUser — the cast is safe.
            login(user as unknown as AuthUser, token, rememberMe);
        ...
        <main className="login-form-panel">
            ...
            <div className="login-form-inner">
                ...
                <p className="login-form-subtitle">
                    Нямаш акаунт?{" "}
                    <Link href="/register">Регистрирай се безплатно →</Link>
                </p>
                {/* Divider */}
                ...
                <p className="login-register-prompt">
                    Нямаш акаунт?{" "}
                    <Link href="/register">Създай безплатен акаунт</Link>
                </p>
            </div>
        </main>
    );
