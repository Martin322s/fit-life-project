...
import { parseTrainingPlanCreate } from "@/src/lib/server/training-plans-validation";
import * as repo from "@/src/lib/server/repositories/training-plans";
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const { items, total } = await repo.list({ page, limit, search, goalType, level, equipment });
    const totalPages = Math.max(1, Math.ceil(total / limit));
    return NextResponse.json({ items, page, limit, total, totalPages });
  } catch (err) {
    console.error("GET /api/training-plans", err);
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const input = parseTrainingPlanCreate(body);
    if (input instanceof NextResponse) return input;

    const item = await repo.create(input);
    return NextResponse.json({ item }, { status: 201 });
  } catch (err) {
    console.error("POST /api/training-plans", err);
    return NextResponse.json({ message: "Server error" }, { status: 500 });
  }
}
