...
type Props = { label: string; value: string; sub: string; accent?: string; accentColor?: string };

export default function TrainingStatCard({ label, value, sub, accent, accentColor }: Props): JSX.Element {
    const isNumeric = /^[\d.,+\-% ]+$/.test(value.trim());
    const fontSize = isNumeric ? "2rem" : value.length > 10 ? "1.1rem" : "1.4rem";

    return (
        <div className="card card-sm tp-card">
