...
export default function ConsistencyCard({ loggedDays, streak }: Props): JSX.Element {
    const logged = loggedDays.filter(Boolean).length;
    const pct = loggedDays.length ? Math.round((logged / loggedDays.length) * 100) : 0;
        ...
        <div className="card wt-card" style={{ display: "flex", flexDirection: "column", gap: "var(--sp-4)" }}>
                ...
                {[0, 1, 2, 3, 4].map((week) => (
                    <div key={week} style={{ display: "grid", gridTemplateColumns: "repeat(7, minmax(0,1fr))", gap: 4, marginBottom: 4 }}>
                        {[0, 1, 2, 3, 4, 5, 6].map((day) => {
                            ...
                            const loggedDay = loggedDays[idx] ?? false;
                            const isToday = idx === loggedDays.length - 1;
                            return (
                                <div key={day} style={{
                                    aspectRatio: "1", borderRadius: 5,
                                    background: loggedDay
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "var(--sp-3)" }}>
                <div style={{ padding: "var(--sp-3)", borderRadius: "var(--r-lg)", background: "rgba(255,255,255,0.025)", border: "1px solid var(--c-border,rgba(255,255,255,0.06))", textAlign: "center" }}>
                    ...
                    <div style={{ fontFamily: "var(--font-display)", fontSize: "1.4rem", fontWeight: 800, color: "var(--c-electric,#0066FF)", marginTop: 4 }}>{logged} / {loggedDays.length}</div>
                </div>
                <div style={{ padding: "var(--sp-3)", borderRadius: "var(--r-lg)", background: "rgba(255,255,255,0.025)", border: "1px solid var(--c-border,rgba(255,255,255,0.06))", textAlign: "center" }}>
